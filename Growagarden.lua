...
local function MakeLoop(toggle, func)
    task.spawn(function()
        while true do
            task.wait(0.5)
            if toggle.Value then pcall(func) end
        end
    end)
end

local function SellCropsInHand()
...
local SellNode = Window:TreeNode({ Title = "Shop 🛒" })
SellNode:Button({ Text = "🛒 Seed Shop", Callback = SeedShop })
SellNode:Button({ Text = "🛒 Hide All Shop", Callback = hide })
